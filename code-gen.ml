...
let primitive_names_to_labels = 
  ...
  and create_row tbl element =
    ...
    | Sexpr(String(x)) -> List.append tbl [(element, (next_offset, (handle_string x)))] 
     (* TODO check if x need to be wrapped with <doubleqoute> ####change also for symbol!!*)
  ...
  let rec generate depth width consts fvars e = (* num of arsgs of current lambda *)
    match e with
      ...
      | Var'(VarParam(name, minor)) -> (* TODO - havn't been tested!! *)
          let debug_start = Printf.sprintf 
      ...
      | Var'(VarBound(name, major, minor)) -> (* TODO - havn't been tested!! *)
          let debug_start = Printf.sprintf 
      ...
      | Set'(Var'(VarParam(name, minor)),exp) -> (* TODO - havn't been tested!! *)
          let debug_start = Printf.sprintf 
      ...
      | Set'(Var'(VarBound(name, major, minor)),exp) -> (* TODO - havn't been tested!! *)
        let debug_start = Printf.sprintf 
      ...
      | Set'(Var'(VarFree(name)),exp) -> (* TODO - havn't been tested!! but this case shouldn't be tested *)
        let debug_start = Printf.sprintf 
      ...
      | BoxGet'(v) -> (* TODO - havn't been tested!! *)
        let debug_start = Printf.sprintf 
          ";;Generate BoxGet'(var):\n\n" in
      ...
      | BoxSet'(v,e) -> (* TODO - havn't been tested!! *)
        let debug_start = Printf.sprintf 
      ...
      | Box'(VarParam(var, minor)) -> (* TODO - havn't been tested!! *)
        let debug_start = Printf.sprintf 
      ...
      | Def'(Var'(v), value) ->
        ...
        (match v with
          ...
          | VarParam(var, minor) -> (* TODO - havn't been tested!! but this case shouldn't be tested *)
            let debug_start = Printf.sprintf 
          ...
          | VarBound(var, major, minor) -> (* TODO - havn't been tested!! but this case shouldn't be tested *)
            let debug_start = Printf.sprintf 
      ...
      | Applic'(rator, rands) ->
        ...
        let varify_rax_is_closure = "" in (* TODO: neccessary?*)
        ...
        let restore_stack = 
          ...
          \n\n" in
        let code = push_magic ^ push_args ^ generate_rator ^ varify_rax_is_closure ^ call ^ restore_stack in
        let debug_end = Printf.sprintf
      ...
      | ApplicTP'(rator, rands) -> 
        ...
        let code = push_magic ^ push_args ^ generate_rator ^ varify_rax_is_closure ^ call in
        let debug_end = Printf.sprintf
